...
### 12.1 Crear datos iniciales con `DatabaseSeeder`

Para no crear manualmente roles, permisos y relaciones cada vez que se prueba
la autorización, agregamos `config/DatabaseSeeder.java`. Spring ejecuta el
`CommandLineRunner` al iniciar la aplicación.

El seeder es idempotente: primero busca cada registro por sus datos únicos y
solo lo crea o asigna cuando aún no existe. Por eso se puede iniciar la
aplicación varias veces sin duplicar los datos.
...
public class DatabaseSeeder {
        ...
        CommandLineRunner seedDatabase() {
                return arguments -> {
                        ...
                        for (PermissionSeed seed : ADMIN_PERMISSIONS) {
                                assignPermission(adminRole, findOrCreatePermission(seed));
                        }

                        assignPermission(consultantRole, findOrCreatePermission(
                                        new PermissionSeed("GET", "/api/users")
                        ));
                        assignPermission(consultantRole, findOrCreatePermission(
                                        new PermissionSeed("GET", "/api/users/{id}")
        ...
        private Permission findOrCreatePermission(PermissionSeed seed) {
                return permissionRepository.findByUrlAndMethod(
                                seed.url(),
                                seed.method()
                ).orElseGet(() -> {
...
Inicia el proyecto con `./mvnw spring-boot:run` o ejecutando
`MsSecurityApplication`. El seeder se ejecuta automáticamente después de que
Spring crea el contexto.
